...
boomingPoint = (locationOfFy1 -
                (timeOfFy1 + timeOfFy1Boom) * directionSpeedOfFy1 +
                np.array([0,0, 1 / 2 * gravity * timeOfFy1Boom * timeOfFy1Boom]))

def centerPointOfBooming(t):
    return boomingPoint + t * np.array([0,0,-3])

def MsPosition(t):
    return locationOfMs1 + (t + timeOfFy1 + timeOfFy1Boom) * directionSpeedOfMs1
...
def dist(a: np.ndarray, b: np.ndarray) -> float:
    """a, b 都是 (3,) 或 (2,) 向量，返回欧氏距离"""
    return float(np.linalg.norm(a - b))

time = 0
hideTime = 0
while time <= dist(locationOfMs1,originPoint)/speedOfMs:
    if point_to_line_distance(locationOfTrue,MsPosition(time),centerPointOfBooming(time)) <= 10:
        hideTime += 0.01
    time += 0.01
